...
(function($) {
	 ...
	 * 统一过滤快速搜索的2头空格
	 */
	function _filterFastQueryParameters(obj) {
	 ...
	 * 过滤script
	 */
	function _filterDataScript (data){
		return data.map(function(currData){
			for (var currObj in currData) {
				if (typeof currData[currObj] == "string") {
					currData[currObj]= currData[currObj].replace(/<(\u0020+|)script[^>]*>(.|\n)*?(?=<(\u0020+|)\/(\u0020+|)script(\u0020+|)>)<(\u0020+|)\/(\u0020+|)script(\u0020+|)>/gi, '');
				}
			}
			return currData;
		});	
		...
		init : function(options, name) {
			...
			var dtGridObject = {
				...
				load : function(){
					...
					// bycj -- 快速搜索去掉空格
					// console.log(" dtGridReflectionObj.fastQueryParameters is :" ,  dtGridReflectionObj.fastQueryParameters);
					_filterFastQueryParameters(this.fastQueryParameters);
								...
								$.ajax({
									...
									success:function(datas){
										dtGridReflectionObj.hideProcessBar(function(){
											...
											dtGridReflectionObj.originalDatas = dtGridReflectionObj.option.ncFilter ?  _filterDataScript ( datas )  : datas;
											dtGridReflectionObj.originalDatas = dtGridReflectionObj.originalDatas?dtGridReflectionObj.originalDatas:new Array();
											//处理基础数据集
											dtGridReflectionObj.baseDatas = dtGridReflectionObj.originalDatas.slice(0, dtGridReflectionObj.originalDatas.length);
											//处理分页属性
					...
					if(!dtGridReflectionObj.option.ajaxLoad||dtGridReflectionObj.option.loadAll){
						...
						// 修改排序条件，支持多参数按顺序排序 by liweidong 2017-09-14 18:12:12
                        ...
                        if(dtGridReflectionObj.sortParameter&&dtGridReflectionObj.sortParameter.columnId){
                            var sorts = dtGridReflectionObj.sortParameter.columnId;

                            dtGridReflectionObj.advanceQueryParameter = new Object();
                            dtGridReflectionObj.advanceQueryParameter.advanceQuerySorts = new Array();
                        	for(var i=0; i<sorts.length; i++){
                                var advanceQuerySort = new Object();
                                if($.fn.DtGrid.tools.startsWith(sorts[i], 'asc_')){
                                    advanceQuerySort.field = sorts[i].replace('asc_', '');
                                    advanceQuerySort.logic = 1;
                                    dtGridReflectionObj.advanceQueryParameter.advanceQuerySorts.push(advanceQuerySort);
                                }

                                if($.fn.DtGrid.tools.startsWith(sorts[i], 'desc_')){
                                    advanceQuerySort.field = sorts[i].replace('desc_', '');
                                    advanceQuerySort.logic = 2;
                                    dtGridReflectionObj.advanceQueryParameter.advanceQuerySorts.push(advanceQuerySort);
                                }
                            }
						...
						$.ajax({
							...
							success:function(pager){
								...
								dtGridReflectionObj.hideProcessBar(function(){
									//处理展示数据和分页相关信息
									dtGridReflectionObj.exhibitDatas =  dtGridReflectionObj.option.ncFilterScript ?  _filterDataScript ( pager.exhibitDatas )  : pager.exhibitDatas;;
									//获取排序数据集备份
									dtGridReflectionObj.sortOriginalDatas = dtGridReflectionObj.exhibitDatas.slice(0, dtGridReflectionObj.exhibitDatas.length);
									//处理分页信息
					...
					if(dtGridReflectionObj.exhibitDatas!=null){
						...
						for(var i=0; i<dtGridReflectionObj.exhibitDatas.length; i++){
							...
							for(var j=0; j<columns.length; j++){
								...
								// update by liweidong 2017-06-14 修改支持加载引用 start
								// var value = dtGridReflectionObj.exhibitDatas[i][columns[j].id];
								var value;
								if (columns[j].id.indexOf(".")>0){
									var ent = dtGridReflectionObj.exhibitDatas[i][columns[j].id.split(".")[0]];
                                    if (ent){
                                        value = dtGridReflectionObj.exhibitDatas[i][columns[j].id.split(".")[0]][columns[j].id.split(".")[1]];
                                    }else{
                                        value = '';
                                    }
								}else{
                                    value = dtGridReflectionObj.exhibitDatas[i][columns[j].id];
								}
                                // update by liweidong 2017-06-14 修改支持加载引用 end
				...
				constructGridToolBar : function(){
					...
					for(var i=0; i<tools.split('|').length; i++){
						...
						if(tool=='faseQuery'){
							...
							$('#'+dtGridReflectionObj.option.toolbarContainer+' .dt-grid-tools').append(content);
							//console.log("dtGridReflectionObj.option.id" ,dtGridReflectionObj.option.id)
							//绑定方法
				...
				faseQuery : function(){
					...
					for(var j=0; j<dtGridReflectionObj.option.columns.length; j++){
                        ...
                        // 增加默认查询条件，该条件在快速查询中不显示，为查询默认条件，使用hideQuery 标记是否默认，hideQueryValue 标记默认值  by liweidong 2017-09-14 14:26:59
                        if(column.hideQuery==true){
                            content += '<input type="hidden" class="form-control" id="'+column.hideQueryType+'_'+column.id+'" name="'+column.hideQueryType+'_'+column.id+'" value="'+column.hideQueryValue+'"/>';
                            continue;
                        }
                        // 增加默认查询条件，该条件在快速查询中不显示，为查询默认条件，使用hideQuery 标记是否默认，hideQueryValue 标记默认值  by liweidong 2017-09-14 14:26:59
                        ...
                        if(column.fastQuery==true){
							...
							if(column.fastQueryType=='range'){
								...
								if(column.type=='date'){
									...
									//bycj for cj 添加一个date数据库中是 unix 时间戳的 搜索
									content += '	<input type="text" class="form-control J-date-start" nc-format="'+ (column.ncFormat ? column.ncFormat :"") +'" id="ge_'+column.id+'" name="ge_'+column.id+'" format="'+column.format+'" placeholder="'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].fastQuery.selectStart+column.title+'" onclick="WdatePicker({dateFmt:\''+$.fn.DtGrid.tools.replaceAll(column.format, 'h', 'H')+'\'});" />';
								...
								if(column.type=='date'){
									content += '<div class="input-group">';
									//bycj for cj 添加一个date数据库中是 unix 时间戳的 搜索
									content += '	<input type="text" class="form-control J-date-end" nc-format="'+ (column.ncFormat ? column.ncFormat :"") +'" id="le_'+column.id+'" name="le_'+column.id+'" format="'+column.format+'" placeholder="'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].fastQuery.selectEnd+column.title+'" onclick="WdatePicker({dateFmt:\''+$.fn.DtGrid.tools.replaceAll(column.format, 'h', 'H')+'\'});" />';
									content += '	<div class="input-group-addon"><i class="fa fa-calendar"></i></div>';
									content += '</div>';
								}else{
				...
				doFaseQuery : function(form){
					...
					var dtGridReflectionObj = this;
					//清空快速查询中的参数
					// 注释掉是因为需要将js中的默认查询条件保留  by liweidong 2017-03-13 14:26:59
					dtGridReflectionObj.fastQueryParameters = new Object();
                    dtGridReflectionObj.pager.nowPage = 1; // 执行查询是初始化分页，指定当前页为第1页，解决在第2+页 进行查询获取不到数据的问题 by liweidong 2017-10-19 11:24:13
					//遍历获取快速查询参数
					var elements = form.elements;
					for(var i=0;i<elements.length;i++){
						var element = elements[i] , 
							//bycj for cj 添加一个date数据库中是 unix 时间戳的 搜索
							ncFormat = element.getAttribute('nc-format');
						if($.fn.DtGrid.tools.equalsIgnoreCase(element.type, 'text') 
							...
							if (element.name != ""){
								//bycj for cj 添加一个date数据库中是 unix 时间戳的 搜索
								if (ncFormat && ncFormat=='longDate') {
									console.log("class i" ,element.classList.contains("J-date-start"));
									dtGridReflectionObj.fastQueryParameters[element.name] = 
										element.value ? 
										( new Date( element.classList.contains("J-date-start") ?  element.value : ( element.value + " 23:59:59" )  ).getTime() +"" )
										:"" ;
									dtGridReflectionObj.fastQueryParameters[element.name+'_format'] = null;
								}else{
					...
					$('#dt_grid_export_execute_'+exportType+'_'+dtGridReflectionObj.option.id).click(function(){
						...
						for(var i=0; i<originalDatas.length; i++){
							...
							$('input[id*=dt_grid_export_check_'+exportType+'_'+dtGridReflectionObj.option.id+'_]:checked').each(function(){
								...
								// update by liweidong 2017-06-14 修改导出支持加载引用 start
                                var value;
                                if (column.id.indexOf(".")>0){
                                    var ent = data[column.id.split(".")[0]];
                                    if (ent){
                                        value = data[column.id.split(".")[0]][column.id.split(".")[1]];
                                    }else{
                                        value = '';
                                    }
                                }else{
								...
								var fieldContent = dtGridReflectionObj.formatContent(column, value);
...
(function($) {
	$.fn.DtGrid.defaultOptions = {
		...
		column : {
			...
			fastQuery : false,
			fastQueryType : '',
			// 增加默认查询条件 by liweidong 2017-09-14 16:54:50
            hideQuery : false, // 隐藏搜索框
			hideQueryValue:'', // 默认值
            hideQueryType:'eq',// 条件类型 默认eq
            // 增加默认查询条件 by liweidong 2017-09-14 16:54:50
			advanceQuery : true,
			'export' : true,
			print : true,
 ...
 */
(function (win, $) {
	var NcDtMini = function (ele, options) {
		$.extend(this, options);
		this.$element = $(ele);
		this.nowPage = 1;
	};
	 ...
	 * 获取数据
	 */
	NcDtMini.prototype.load = function () {
		var self = this,
			emptyUl = '<div class="row">没有符合条件的记录</div>'
			;
		self.$panelSearch = $("<div>", {
			"class": "dtmini-search-panel",
			html: '<div class="input-group"><input type="text" class="form-control" value="' +
			(self.fastQueryParameters.lk_moduleName ? self.fastQueryParameters.lk_moduleName : "") +
			'"><div class="input-group-btn"><button type="button" class="btn btn-success" data-nc-module-search>查找模块</button></div></div>'
		});
		self.$resultDiv = $("<div>", {
			'class': "dtmini-resutl-panel clearfix"
		});

		self.$ulSort = $("<ul>", {
			'class': 'dtmini-result-ul'
		});
		self.$pageUl = $("<ul>", {
			"class": 'pagination pagination-sm dt-grid-pager-button'
		});


		$.post(self.loadURL, self._getPostData(), function (xhr) {
			var _isOk = (xhr &&xhr.exhibitDatas &&xhr.exhibitDatas.length) ? true :false ;
			self.dtObj = xhr;
			self.$element.empty();
			self.$resultDiv.append(_isOk ? self.$ulSort : emptyUl );
			self.$element.append(self.$panelSearch, self.$resultDiv, self.$pageUl);
			//添加分页按钮
			_isOk &&self._buildPage()

			self._bindEvents();
			//callback running
			$.type(self.callback) == 'function' && self.callback(self.dtObj.exhibitDatas, self.$ulSort)
		});
	};
	/**
	 *
	 * @private
	 */
	NcDtMini.prototype.goPage = function (nowPage) {
	 ...
	 * @private
	 */
	NcDtMini.prototype._filterXhrData = function () {
	 ...
	 * @private
	 */
	NcDtMini.prototype._buildPage = function () {
		var self = this,
			nowPage = self.dtObj.nowPage,
			pageCount = self.dtObj.pageCount;

		console.log("当前页号:", nowPage);
		console.log("共有页号 ：", pageCount)

		//第一页按钮
		var goFirstBtn = $("<li>", {
			"class": self.nowPage <= 1 ? 'disabled' : "",
			html: '<a href="javascript:void(0);"><i class="fa fa-angle-double-left"></i></a>'
		}).click(function () {
			console.log("第一页按钮点击");
		...
		self.$pageUl.append(goFirstBtn);
		//上一页按钮

		var goPrevBtn = $("<li>", {
			"class": self.nowPage <= 1 ? 'disabled' : "",
			html: '<a href="javascript:void(0);"><i class="fa fa-angle-left"></i></a>'
		}).click(function () {
			if ($(this).hasClass("disabled")) {
				return;
			}
			self.goPage(nowPage - 1);
			console.log("上一页按钮 click");
		});
		self.$pageUl.append(goPrevBtn);
		...
		if (pageCount <= 5) {
			for (var i = 1; i <= pageCount; i++) {
				...
				self.$pageUl.append(goPageBtn);
				if (i != nowPage) {
					goPageBtn.click(function () {
						console.log("跳转页面");
						self.goPage($(this).attr('page'));
					});
				}
			}
		} else {
		...
		//下一页按钮
		var goNextBtn = $("<li>", {
			'class': nowPage >= pageCount ? 'disabled' : '',
			html: '<a href="javascript:void(0);"><i class="fa fa-angle-right"></i></a>'
		});
		self.$pageUl.append(goNextBtn);
		...
		var goLastBtn = $("<li>", {
			'class': nowPage >= pageCount ? 'disabled' : '',
			html: '<a href="javascript:void(0);"><i class="fa fa-angle-double-right"></i></a>'
		});
		self.$pageUl.append(goLastBtn);
		goLastBtn.click(function () {
			if ($(this).hasClass("disabled"))return;
			// dtGridReflectionObj.loadByPage('last');
			console.log("最后一页按钮");
			self.goPage(pageCount);
		});
	 ...
	 * @private
	 */
	NcDtMini.prototype._bindEvents = function () {
		var self = this;
		self.$panelSearch.find("[data-nc-module-search]").click(function () {
			console.log("点击搜索");
			var keyword = $.trim(self.$panelSearch.find("input").val())
				;
			if (keyword != '') {
				self.fastQueryParameters.lk_moduleName = keyword;
			} else {
				if (self.fastQueryParameters.lk_moduleName) delete self.fastQueryParameters.lk_moduleName;
			}
			self.nowPage = 1;
			self.load();
	 ...
	 * @private
	 */
	NcDtMini.prototype._getPostData = function () {
		var self = this;

		dtGridPager = {
			ncColumnsType: self.ncColumnsType,
			isExport: false,
			...
			pageCount: -1,
			parameters: {},
			fastQueryParameters: self.fastQueryParameters,
			advanceQueryConditions: [],
			advanceQuerySorts: []
		};
		return {
			dtGridPager: JSON.stringify(dtGridPager)
		}
	};
	 ...
	 * 默认配置
	 */
	NcDtMini.defaultOptions = {
		loadURL: "",
		pageSize: 2,
		ncColumnsType: {
			int: ["moduleId", "moduleShow"]
		},
		fastQueryParameters: {},
	...
	function Plugin(option) {
		...
		this.each(function () {
			var $this = $(this),
				data = $this.data('nc.dtMini'),
				options = $.extend({}, NcDtMini.defaultOptions, $this.data(), typeof option == 'object' && option)
			if (!data) $this.data('nc.dtMini', (data = new NcDtMini(this, options)));
	...
	$.fn.ncDtMini = Plugin;
	$.fn.ncDtMini.Constructor = NcDtMini;
